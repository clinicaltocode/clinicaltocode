...
export default async function HomePage() {
  ...
  return (
    <>
      {/* Masthead */}
      <div className="border-b border-[#e0dcd5]">
        <div className="mx-auto px-6 max-w-[1200px] py-12 text-center">
          <p className="text-sm uppercase tracking-[0.2em] text-[#6b6b6b] mb-3">Where Healthcare Meets Technology</p>
          <h1 className="font-serif text-4xl md:text-5xl font-bold text-[#1a1a1a] leading-tight">
            Real insights from clinicians,<br className="hidden md:block" /> for those building the future of care.
          </h1>
        </div>
      </div>

      <main className="mx-auto px-6 max-w-[1200px] py-12">
        {featured ? (
          <>
            ...
            <article className="mb-16">
              <Link href={`/articles/${featured.slug}`} className="group grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                {featured.coverImage?.asset?.url ? (
                  ...
                  <div className="aspect-[3/2] bg-[#f4f1ec]" />
                )}
                <div>
                  {featured.category && (
                    <span className="text-xs font-semibold uppercase tracking-wider text-primary">{featured.category.title}</span>
                  )}
                  <h2 className="font-serif text-3xl md:text-4xl font-bold text-[#1a1a1a] leading-tight mt-2 group-hover:text-primary transition-colors">
                    {featured.title}
                  </h2>
                  {featured.excerpt && (
                    <p className="text-[#6b6b6b] mt-4 text-lg leading-relaxed line-clamp-3">{featured.excerpt}</p>
                  )}
            ...
            {/* Divider */}
            <div className="border-t border-[#e0dcd5] mb-12" />
                ...
                {rest.map(article => (
                  <article key={article._id}>
                    <Link href={`/articles/${article.slug}`} className="group block">
                      ...
                      {article.category && (
                        <span className="text-xs font-semibold uppercase tracking-wider text-primary">{article.category.title}</span>
                      )}
                      <h3 className="font-serif text-xl font-bold text-[#1a1a1a] leading-snug mt-1 group-hover:text-primary transition-colors">
                        {article.title}
                      </h3>
